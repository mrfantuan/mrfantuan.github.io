---
layout: post
title: Golang tutorial series - 28.多态
date:  2018-06-13 17:30:00
categories: Golang
tag: Go
---

Go 通过[接口](../go-18-interface1)来实现多态。我们已经讨论过，在 Go 语言中，我们是隐式地实现接口。一个类型如果定义了接口所声明的全部[方法](../go-17-function)，那它就实现了该接口。现在我们来看看，利用接口，Go 是如何实现多态的。
...
上面的[函数](../go-6-function)接收一个 `Income` 接口类型的[切片](../go-11-arrary-slice)作为参数。该函数会遍历这个接口切片，并依个调用 `calculate()` 方法，计算出总收入。该函数同样也会通过调用 `source()` 显示收入来源。根据 `Income` 接口的具体类型，程序会调用不同的 `calculate()` 和 `source()` 方法。于是，我们在 `calculateNetIncome` 函数中就实现了多态。
